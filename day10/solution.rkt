...
(define (lp-solve buttons joltage)
  ...
  (define params (for/list ([i (in-range M)]) (format "x~a" i)))
  ; Objective function
  (define objective (format "min: ~a;" (string-join params " + ")))
  (define constraints
    (for/list ([sum (in-list joltage)][i (in-range N)])
      (~> 
  ...
  (define program (string-join (list objective
                                     (string-join constraints "\n")
                                     types) "\n"))
  (define exe "/opt/homebrew/bin/lp_solve")
  (define output (with-output-to-string (λ ()
                   (system (format "echo \"~a\" | ~a -S1 /dev/stdin" program exe)))))
  (~> output
