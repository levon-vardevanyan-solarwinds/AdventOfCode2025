...
(define deltas (~> (cartesian-product '(0 1 -1) '(0 1 -1)) (remove '(0 0) _)))

(define (adjacent posn)
  (~> deltas (map (λ (D) (map + D posn)) _) (filter in-bounds? _)))
...
;; part two
(for/sum ([i (in-naturals)] #:do [(define tr (to-remove rolls))
                                  (define len (set-count tr))]
                            #:break (zero? len))
         (set-subtract! rolls tr)
         len)
