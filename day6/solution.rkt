...
(match-define (list head ... tail) (file->lines "input.txt"))
...
(define (part-1)
  (define nums
    (map (λ~> string-split (map string->number _) list->vector) head))

  (for/sum ([op (in-list ops)] [i (in-naturals)])
...
(define (part-2)
  ...
  (for/sum ([op (in-list ops)] [args (in-list nums)]) (apply op args)))
